...
def build_stores(env: simpy.Environment, stores_dict: dict, store_configs: List[StoreConfig], 
    ...
    for cfg in store_configs:
        ...
        res = simpy.Container(env, capacity=cfg.capacity, init=opening)
        # Store the key on the container for later retrieval
        setattr(res, 'store_key', cfg.key)
        stores_dict[cfg.key] = res
